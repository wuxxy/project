...
func ParseCodeJWT(tokenString string, serviceSecret string) (string, string, error) {
	...
	// Optional: type check
	if typ, ok := claims["type"].(string); ok && typ != "code" {
		return "", "", errors.New("not a code token")
	}

	sub, ok1 := claims["sub"].(string)
	state, ok2 := claims["s"].(string)
	if !ok1 || !ok2 {
		return "", "", errors.New("missing required claim(s)")
	}
