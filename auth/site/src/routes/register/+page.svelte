<script lang="ts">
    ...
    function resetCaptcha() {
        if (window.turnstile && turnstileEl) {
            ...
            turnstileToken = ''; // Clear token on reset
        }
    }
    ...
    function submit() {
        ...
        client.post('/auth/register', {
            ...
            .catch((error) => {
                ...
                serverError = error.response?.data?.error || 'Server error';

                if (error.response?.data?.errors) {
                    const errs = error.response.data.errors;
                    if (errs.username) usernameErrors.push(errs.username);
                    if (errs.password) passwordErrors.push(errs.password);
                    if (errs.email) emailErrors.push(errs.email);  // <--- this line adds "Invalid email format"
                } else {
                    usernameErrors = [''];
                    passwordErrors = [''];
                    emailErrors = [''];
...
<div class="h-full flex flex-col items-center justify-center px-6 py-12">
        <div class="w-full max-w-md relative rounded-t-xl border border-rosebrand-700 bg-black/30 text-white overflow-hidden">
            ...
            <div class="relative z-10 flex flex-row w-full">
                <a
                        href={`/login${redirectUrl ? `?r=${encodeURIComponent(redirectUrl)}` : ''}`}
